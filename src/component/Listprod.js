...
const handelData =(name,description)=>{
    alert (`|le nom est ${name} le descrption est ${description}`)}

    return (
        <div>
           { moviesData.map((Prod,i)=>(
<Child key={i} Prod={Prod} handelData={handelData}/>
           ))}
        </div>
    )
}
